...
class VTUAVDataset(torch_data.Dataset):

    def __init__(self, img_dir, transform=None, random_rect_value=40, block_size=256, img_suffix="_img.png",
                 mask_suffix="_mask.png", enable_cache=False, test_mode=False):
        """
        遥感数据集 torch Dataset
        ...
        for file in files_list:
            img_path_list.append(glob.glob(os.path.join(img_dir, file) + '//mask//ir//*.png'))

        if 0 == len(img_path_list):
            raise RuntimeError("can not find any image in %s" % img_dir)

        def gen_label_path(img_path):
            return img_path.replace('//mask', '').replace('png', 'jpg')
        ...
        for img_paths in img_path_list:
            for img_path in img_paths:
                self.data_list.append({
                    'mask_path': img_path,
                    'img_path': gen_label_path(img_path),
                })
    ...
    def __getitem__(self, index):
        ...
        #
        # new_img[new_img_top:new_img_top + new_height, new_img_left:new_img_left + new_width] = img[
        #                                                                                        ori_img_top:ori_img_bottom,
        #                                                                                        ori_img_left:ori_img_right]
        # new_mask[new_img_top:new_img_top + new_height, new_img_left:new_img_left + new_width] = mask[
        #                                                                                         ori_img_top:ori_img_bottom,
        #                                                                                         ori_img_left:ori_img_right]
